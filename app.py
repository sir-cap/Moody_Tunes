...
# Defining emotion labels and the variable to save the detected emotion
        ...
        if check_mood_button:
            ...
            if countdown_start:
                ...
                if countdown_start and time.time() >= countdown_end_time:
                    ...
                    st.write("Capturing your mood...")

                    # Get the list of files in the pictures folder
                    pictures_folder = "pictures"
                    os.makedirs(pictures_folder, exist_ok=True)
                    files = os.listdir(pictures_folder)

                    if len(files) > 0:
                        # Sort the files by their modified time in descending order
                        sorted_files = sorted(files, key=lambda x: os.path.getmtime(os.path.join(pictures_folder, x))),
                        reverse=True

                        # Read the latest image file
                        image_path = os.path.join(pictures_folder, sorted_files[0])
                        ...
                        if detected_emotion is not None:
                            ...
                            moody_tunes(pictures_folder, detected_emotion, songs_df)
